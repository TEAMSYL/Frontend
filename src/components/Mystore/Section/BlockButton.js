...
const BlockButton = ({ userId, account }) => {
  ...
  const handleClick = async () => {
    if (storeSellCount < 10) alert('아직 상품 판매 횟수가 10회 미만입니다.');
      ...
      let json = `{"name":"${userEmail}","description":"${userEmail}님의 ${storeName} 상점에 대한 블록마켓 보증서입니다.","image":"${dataurl}","userId":"${userId}","attributes":[{"trait_type": "Unknown","value": "Unknown"}]}`;

      const ipfsJson = await client.add(json);
      const JsonURL =
        process.env.REACT_APP_INFURA_PROJECT_GATEWAY + `ipfs/${ipfsJson.path}`;
      ...
      let newTokenId = await Erc721Contract.methods
        .mintNFT(account, JsonURL, true)
        .send();
        ...
        <>
          <Box>
            ...
            <Typography
              ...
              fontSize={'13px'}
            >
              당신의 상점명과 아이디를 가져오기 때문에 신중하게 선택해주세요 !
              <br />
              아래 사진은 예시입니다.
            </Typography>
          </Box>
          <Box>
            <canvas id="mycanvas" ref={canvas} width={640} height={366} />
          </Box>
        </>
      ) : (
        <>
          <Box>
            ...
            <Box sx={{ display: 'flex' }}>
              ...
              <Button
                ...
                sx={{ marginLeft: '50px', backgroundColor: 'lightblue' }}
              >
                내 버튼 확인
              </Button>
            </Box>
          ...
          <Box>
            <canvas id="bye" ref={canvas} width={0} height={0} />
          </Box>
        </>
      )}
