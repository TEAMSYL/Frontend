...
const TrackingNumModal = ({open, onClose, request, fetchRequests}) => {
    ...
    const [registLoading, setRegistLoading] = React.useState(false);
    ...
    const handleInquire = async () => {
        ...
        if (data.status == false) {
            ...
            setSuccess(true);
            setFail(false);
            setInfoText("조회 성공. 등록하기 버튼을 누르세요.");
            return;
        } else {
            setSuccess(false);
            setFail(true);
    ...
    const handleRegist = async () => {
        ...
        if (response.status === 200) {
            setInfoText("등록이 완료되었습니다.");
            setRegistSuccess(true);
            setRegistFail(false);
            await fetchRequests();
        } else {
            setInfoText("등록에 실패하였습니다. 다시 시도해주세요.");
            setRegistSuccess(false);
            setRegistFail(true);
        }
    };
                    ...
                    <Grid container rowSpacing={1} sx={{ paddingBottom: '30px'}}>
                        ...
                        <Typography 
                            ...
                            sx={{ 
                                width: '100%',
                                color: (
                                    loading || registLoading ? '#757575' :
                                    fail || registFail ? '#D80C18' :
                                    success || registSuccess ? '#03C75A': '#212121')
                            }}
                        >
                        {infoText}
                        </Typography>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'end', width: '100%', paddingTop: '20px'}}>
                            ...
                            >
                                {   
                                        registLoading ? <CircularProgress size={20} sx={{ color: '#757575'}}/> :
                                        !registLoading && !registFail && !registSuccess ? <Typography sx={{ fontSize: '14px', color: '#212121'}}>등 록</Typography> :
...
export default TrackingNumModal;