import { Avatar, ImageList, ImageListItem, ImageListItemBar, Typography, styled, Box} from '@mui/material';
...
const Honor = ({})=> {
    ...
    const getHonerTokens = async () => {
        try{
            ...
            const onHonerLength = await Erc721Contract.methods.getOnHonerTokenArrayLength().call();
    ...
    return (
        <Box sx={{ width: 1062, margin: '0 auto 100px' }}>
            <StyledBox
            sx={{
        ...
        <ImageList sx={{ width: 1062, height: 640, margin: '0 auto 100px' }}>
        {itemData && itemData.map((item, i) => (
            <ImageListItem key={i}> 
            <Avatar src={item.img} sx={{ width:"520px", height:'316px'}} alt={item.title} variant="square"/>
            <ImageListItemBar
                title={item.title}
                subtitle={<Typography sx={{fontSize:'8px'}}>by: {item.author}</Typography>}
